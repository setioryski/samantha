...
const TherapistModal = ({ therapist, onClose, onSave }) => {
    ...
    useEffect(() => {
        if (therapist) {
            ...
            setFeePercentage(therapist.feePercentage || 0);
        }
    }, [therapist]);

    const handleSubmit = (e) => {
        e.preventDefault();
        onSave({ name, feePercentage });
    };
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-40">
            <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                ...
                <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                    ...
                    <input type="number" value={feePercentage} onChange={(e) => setFeePercentage(e.target.value)} placeholder="Fee Percentage" className="w-full p-2 border rounded" required min="0" max="100" />
